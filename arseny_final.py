...
def arsenyCode(image):
    if image is None:
        return np.zeros((480, 640, 3), np.uint8)  # safety

    height, width = image.shape[:2]
    output = image.copy()  # always work on a copy
    ...
    lines = cv.HoughLinesP(edges.shape, rho=1, theta=np.pi/180, threshold=40,
                            minLineLength=200, maxLineGap=120)
    ...
    def draw_extended_line(fitted_line, color):
        ...
        lefty  = int((-x0 * vy / vx) + y0)
        righty = int(((width - x0) * vy / vx) + y0)
        cv.line(output, (width-1, righty), (0, lefty), color, 3)
        return int(x0), int(y0)
    ...
    if lines is not None:
        for x1,y1,x2,y2 in lines[:,0]:
            ...
            my = (y1 + y2) // 2
            points.append([mx, my])
            cv.circle(output, (mx, my), 6, (0, 255, 255), -1)

    # MAIN FIX: Always return something, even if no lines!
    if len(points) < 4:
        cv.putText(output, "NO LINES DETECTED", (50, height//2),
                   cv.FONT_HERSHEY_DUPLEX, 1.2, (0, 0, 255), 3)
        return output  # EARLY RETURN HERE — THIS IS THE KEY!

    # Only try clustering if we have enough points
    try:
        points_np = np.float32(points)
        criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 10, 1.0)
        _, labels, centers = cv.kmeans(points_np, 2, None, criteria, 10, cv.KMEANS_PP_CENTERS)

        centroids = []
        colors = [(255, 255, 0), (0, 255, 255)]  # cyan, yellow

        for i in range(2):
            group = points_np[labels.ravel() == i]
            if len(group) < 2:
                continue
            line = cv.fitLine(group, cv.DIST_L2, 0, 0.01, 0.01)
            centroid = draw_extended_line(line, colors[i])
            if centroid:
                centroids.append(centroid)
                cv.circle(output, centroid, 10, (255, 0, 255), -1)

        if len(centroids) == 2:
            (x1,y1), (x2,y2) = centroids
            mx, my = (x1+x2)//2, (y1+y2)//2
            cv.circle(output, (mx, my), 12, (0, 0, 255), -1)
            cv.putText(output, "CENTER", (mx+20, my), cv.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 3)

            # Optional: draw vertical center guide
            cv.line(output, (mx, 0), (mx, height), (0, 255, 0), 2)

    except Exception as e:
        print("Processing error:", e)

    return output  # ALWAYS returns a valid frame